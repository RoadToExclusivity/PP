...
std::shared_ptr<ThreadPool> pool = std::shared_ptr<ThreadPool>(); //пока не инициализирован
...
void draw_3d_model_simple(ModelPtrArray const& models, TGAImage &frame, float *zbuffer)
{
    ...
    for (auto const& pModel : models) {
		pool->RunAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile1));
        pool->RunAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile2));
		pool->RunAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile3));
		pool->RunAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile4));
...
int main(int argc, char* args[]) {
    if (2>argc) {
        std::cerr << "Usage: " << args[0] << " obj/model.obj" << std::endl;
        return 1;
    }

    ModelPtrArray models;
    for (int m=1; m<argc; m++) {
        const char *filePath = args[m];
	...
	pool = std::shared_ptr<ThreadPool>(new ThreadPool(THREAD_COUNT));
	...
	pool.reset(); //для корректного thread.join()
    return 0;
}
